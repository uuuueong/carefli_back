...
public interface MessageRepository extends JpaRepository<Message, Long> {
    ...
    @Query("SELECT m FROM Message m WHERE m.connection IS NULL OR m.connection.connectionId = :connectionId ORDER BY m.updatedAt DESC")
    List<Message> findByConnectionIdOrderByUpdatedAtDesc(@Param("connectionId") Long connectionId);
